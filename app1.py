...
async def stream_answer(
    ...
    try:
        ...
        if language and language.lower().startswith("ar"):
            prompt_header = "STRICT RULE: Always answer ONLY in Arabic...\n" + prompt_header
        elif language and language.lower().startswith("en"):
            prompt_header = "STRICT RULE: Always answer ONLY in English...\n" + prompt_header
        ...
        if language and language.lower().startswith("ar"):
            system_content += "\nSTRICT RULE: You MUST ALWAYS respond in Arabic only.\n"
        elif language and language.lower().startswith("en"):
            system_content += "\nSTRICT RULE: You MUST ALWAYS respond in English only.\n"
        ...
        SENT_END = re.compile(r'(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?|\!)\s+')
        ...
        async def generate_audio(sentence, sentence_id):
            """Generate and stream audio for a sentence."""
            try:
                clean_sentence = prepare_for_tts(sentence)
                communicate = edge_tts.Communicate(clean_sentence, voice=voice)

                yield f"data: {safe_json_dumps({'type': 'audio_start', 'sentence_id': sentence_id})}\n\n"
                ...
                async for chunk in communicate.stream():
                    if chunk["type"] == "audio":
                        ...
                        if hexstr != last_chunk:
                            yield f"data: {safe_json_dumps({'type': 'audio_chunk', 'sentence_id': sentence_id, 'chunk': hexstr})}\n\n"
                            last_chunk = hexstr

                yield f"data: {safe_json_dumps({'type': 'audio_complete', 'sentence_id': sentence_id})}\n\n"

            except Exception as e:
                yield f"data: {safe_json_dumps({'type': 'audio_error', 'sentence_id': sentence_id, 'error': str(e)})}\n\n"
        ...
        async def event_stream():
            ...
            async for chunk in stream:
                ...
                if len(buffer) > max_buffer_size:
                    answer_so_far += buffer
                    sentence_counter += 1
                    yield f"data: {safe_json_dumps({'type': 'partial', 'partial': buffer, 'sentence_id': sentence_counter})}\n\n"
                    async for audio_event in generate_audio(buffer, sentence_counter):
                        yield audio_event
                    buffer = ""
                ...
                while True:
                    ...
                    sentence_counter += 1
                    yield f"data: {safe_json_dumps({'type': 'partial', 'partial': sentence, 'sentence_id': sentence_counter})}\n\n"
                    async for audio_event in generate_audio(sentence, sentence_counter):
                        yield audio_event
                    buffer = buffer[sentence_end:]
        ...
        async def error_stream():
            yield f"data: {safe_json_dumps({'type': 'error', 'error': f'Stream error: {str(e)}'})}\n\n"
            yield f"data: {safe_json_dumps({'type': 'done'})}\n\n"
